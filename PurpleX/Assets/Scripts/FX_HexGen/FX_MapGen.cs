 ...
 *      3. Scale: The size of each hex unit.  
 * 		4. Rotate Hex : Determines the rotation of the hex mesh;
...
public class FX_MapGen : MonoBehaviour {
	...
	Vector2 GetHexSize(){
		...
		if(!RotateHex){
			Temp = GetComponent<FX_HexGen>().MakeHex(30, (Scale / 2));
		}else{
			Temp = GetComponent<FX_HexGen>().MakeHex(0, (Scale / 2));
		}
	...
	void GenerateMapRotA(Transform Map){//Generate a map with the pointy part of the Hex on top
		...
		for(int x = 0; x < MapWidth; x++){
			for(int y = 0; y < MapHeight; y++){
				...
				Transform h = GetComponent<FX_HexGen>().MakeHex(30,(Scale / 2)).transform;
	...
	void GenerateMapRotB(Transform Map){ //Generate a map with the flat part of the Hex on top
		...
		for(int x = 0; x < MapWidth; x++){
			for(int y = 0; y < MapHeight; y++){
				...
				Transform h = GetComponent<FX_HexGen>().MakeHex(0,(Scale / 2)).transform;
	...
	void SetHexInfo(int x, int y, Transform h){
		...
		if(y > 1){
			newX = Mathf.CeilToInt(x - (y * (Scale / 2)));
		}
