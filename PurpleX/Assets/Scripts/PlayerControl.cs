...
public class PlayerControl : MonoBehaviour {
    ...
    void Update()
    {
        ...
        money += speed * Time.deltaTime * Mathf.Sqrt(worker);
        if (oldAmountMoney < (int)money) {
            oldAmountMoney = (int)money;
            updateHUD();
        }

        p1UnitsFloat += Time.deltaTime * Random.Range(0, 20) * 0.001f * Mathf.Sqrt(worker);
    ...
    void OnGUI() {
        ...
        if (showOwn) {              
            ...
            if (GUI.Button(new Rect(left + 5, 30, 130, 20), "Build Worker ("+priceWorker+")") && money >= priceWorker) {
                ...
                worker++;
                if (worker % 5 == 0) priceWorker += worker;
                updateHUD();
            }
            // Unit button
            if (money < priceUnit) {
    ...
    void updateHUD() {
        hud.text = "Money: " + oldAmountMoney + "  Units: " + ownUnits + "  Worker: " + worker;
    }
}
