...
public class HexGrid : MonoBehaviour {
    ...
    private void _deleteAllUnits() {
        unselect();
        HexPosition.clear("Unit");
    }

    private void _spawnOwnUnits(GameObject parent) {
        for (int i = 0; i < GameMaker.Instance.units; i++) {
            Vector3 spawnPoint = new Vector3((i - (GameMaker.Instance.units / 2)) * 1.5f, 0, -1 * Random.Range(3, 6));
    ...
    private void _spawnOtherUnits(GameObject parent) {
        int spawnUnits = GameMaker.Instance.enemysUnits[GameMaker.Instance.selectedEnemy];
    ...
    void OnGUI() {
        if (gameOver) {
            ...
            GameMaker.Instance.units = _getNumUnits(0);
            if (GameMaker.Instance.units < 1) {
                GameMaker.Instance.units = 1;
            }
            int se = GameMaker.Instance.selectedEnemy;
            GameMaker.Instance.enemysUnits[se] = _getNumUnits(1);
            if (_getNumUnits(1) <= 0) {
                GameMaker.Instance.enemysConquered[se] = true;
            }
